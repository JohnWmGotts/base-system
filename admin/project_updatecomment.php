<?php
	...
	$pagetitle="Project Update Comment";
	$tbl_nm = "projectupdatecomment";
	//$id	=	"basicsId";
	...
	if(isset($_GET['action']) && $_GET['action']=='inactive') {
		...
		$con->recordselect("UPDATE projectupdatecomment SET updatecommentstatus = 0 WHERE updatecommentId='".$_GET['id']."'");		
		redirect(SITE_ADM."project_updatecomment.php?msg=SUCBLO&page=".$page1);
	}
	if(isset($_GET['action']) && $_GET['action']=='active') {
		
		$con->recordselect("UPDATE projectupdatecomment SET updatecommentstatus = 1 WHERE updatecommentId='".$_GET['id']."'");		
		redirect(SITE_ADM."project_updatecomment.php?msg=SUCACT&page=".$page1);
	}
	
		 $sqlQuery = "select p.*, pco.*, us.* from projects as p, projectupdatecomment as pco, users as us where p.published=1 and p.accepted!=3 and p.projectId=pco.projectId and pco.userId=us.userId order by pco.updateCommentTime DESC";
	//$results = $con->select($sqlQuery,$page,$perpage,15,2,0);
	$results = $con->recordselect($sqlQuery);
	$content="project_updatecomment";
