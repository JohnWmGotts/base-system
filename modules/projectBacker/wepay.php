...
class WePay {
	...
	public static function getToken($code, $redirect_uri) {
		...
		curl_setopt($ch, CURLOPT_TIMEOUT, 60); // 5-second timeout, adjust to taste
		curl_setopt($ch,CURLOPT_RETURNTRANSFER,1);
		curl_setopt($ch,CURLOPT_POST,1);
		curl_setopt($ch, CURLOPT_URL, $uri);
		curl_setopt($ch, CURLOPT_POSTFIELDS, $params);
		curl_setopt($ch,CURLOPT_SSL_VERIFYPEER,true); 
		curl_setopt($ch,CURLOPT_CAINFO,'/home/sparkova/public_html/modules/projectBacker/mozilla.pem');
		$raw = curl_exec($ch);
	...
	public function request($endpoint, array $values = array()) {
		if (!$this->ch) {
			...
			curl_setopt($this->ch,CURLOPT_CAINFO,'/home/sparkova/public_html/modules/projectBacker/mozilla.pem');
			curl_setopt($this->ch, CURLOPT_TIMEOUT, 60); // 5-second timeout, adjust to taste
