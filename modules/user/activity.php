<?php
	...
	if(!isset($_GET['user']) || (isset($_GET['user']) && $_GET['user']==''))
	{
		...
		$sel_2=$con->recordselect("SELECT *,cm.commentTime as sorting FROM `projectcomments` as cm,`projects` as p WHERE cm.userId ='".$_SESSION['userId']."' AND cm.projectId=p.projectId AND p.accepted!='3' order by cm.commentTime desc");													
	...
	else
	{
		...
		$sel_2=$con->recordselect("SELECT *,cm.commentTime as sorting FROM `projectcomments` as cm,`projects` as p1 WHERE cm.userId ='".$_GET['user']."' AND cm.projectId=p1.projectId AND p1.accepted!='3' order by cm.commentTime desc");													
		$total_count += mysql_num_rows($sel_2) ;
		...
		$sel_4=$con->recordselect("SELECT *,upct.updateCommentTime as sorting FROM `projectupdatecomment` as upct,`projects` as p1 WHERE upct.userId ='".$_GET['user']."' AND upct.projectId=p1.projectId AND p1.accepted!='3' AND upct.updatecommentstatus=1 order by upct.updateCommentTime desc");	
		$total_count += mysql_num_rows($sel_4) ;
		...
		$total_count += mysql_num_rows($sel_6) ;
		/*$sel_1=$con->recordselect("SELECT *,backingTime as sorting FROM `projectbacking` WHERE `userId` ='".$_GET['user']."' group by projectId order by backingTime desc");													
		$total_count = mysql_num_rows($sel_1) ;
				
		$sel_2=$con->recordselect("SELECT *,commentTime as sorting FROM `projectcomments` WHERE `userId` ='".$_GET['user']."' order by commentTime desc");													
		$total_count += mysql_num_rows($sel_2) ;
				
		$sel_3=$con->recordselect("SELECT *,updateTime as sorting FROM `projectupdate` WHERE `userId` ='".$_GET['user']."' order by updateTime desc");	
		$total_count += mysql_num_rows($sel_3) ;
		
		$sel_4=$con->recordselect("SELECT *,updateCommentTime as sorting FROM `projectupdatecomment` WHERE `userId` ='".$_GET['user']."' order by updateCommentTime desc");	
		$total_count += mysql_num_rows($sel_4) ;
		
		$sel_5=$con->recordselect("SELECT *,pbs.projectStart as sorting FROM projects as p, projectbasics as pbs WHERE p.userId ='".$_GET['user']."' and p.projectId=pbs.projectId and p.published=1 and p.accepted=1 order by pbs.projectStart desc");	
		$total_count += mysql_num_rows($sel_5) ;*/
	}
	...
	require_once("activity_pagination_recent.php");	
	//$categoriesList = $con->recordselect("SELECT * from categories limit 10");	
	$title = "Activity";
	$meta = array("description"=>"Activity","keywords"=>"Activity");
