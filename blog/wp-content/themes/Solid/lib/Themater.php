...
class Themater
{
    ...
    */
    ...
    function _definitions()
    {
        ...
        if(!defined('THEMATER_URL')) {
            define('THEMATER_URL',  get_bloginfo('template_directory') . '/lib');
        }
        ...
        if(!defined('THEMATER_INCLUDES_URL')) {
            define('THEMATER_INCLUDES_URL',  get_bloginfo('template_directory') . '/includes');
        }
    ...
    */
    ...
    function admin_option($menu, $title, $name = false, $type = false, $value = '', $attributes = array())
    {
        if(is_admin()) {
            ...
            if($name && $type) {
               ...
               if(isset($this->options) && isset($this->options['theme_options']) && $this->options['theme_options'] && !isset($this->options['theme_options'][$name]) ) {
                ...
                if(!isset($attributes['priority'])) {
					// jwg fixups
					if (!isset($this->options['admin_options_priorities'])) $this->options['admin_options_priorities'] = array();
					if (!isset($this->options['admin_options_priorities'][$menu_title])) $this->options['admin_options_priorities'][$menu_title] = array();
					if (!isset($this->options['admin_options_priorities'][$menu_title]['priority'])) $this->options['admin_options_priorities'][$menu_title]['priority'] = 0;
    ...
    function display_widget($widget,  $instance = false, $args = array('before_widget' => '<ul class="widget-wrap"><li id="%1$s" class="widget %2$s">','after_widget' => '</li></ul>', 'before_title' => '<h3 class="widgettitle">','after_title' => '</h3>')) 
    {
        $widget_name = $widget;
        $custom_widgets = array('Banners125', 'Posts', 'Comments', 'InfoBox', 'SocialConnect', 'SocialShare', 'Tabs');
...
$this->_load_shortcodes();
    ...
    function _load_general_options()
    {
        ...
        if($this->display('custom_css') || (isset($this->options['custom_css']) && $this->options['custom_css'])) {
