<?php 
//require_once(DIR_INC.'paypal/config.php');  		// for old api use 
require_once (DIR_INC.'paypal/paypal.class.php');  // for old api use (still required)
require_once (DIR_INC.'paypal/payment.class.php');	// updated for new api
require_once (DIR_INC.'paypal/config.php'); // altered to set global vars from new configuration.php
...
if (!function_exists('unsanitize_string')) {
...
function get_ip_address1() {
	foreach (array('HTTP_CLIENT_IP', 'HTTP_X_CLIENT_IP', 'HTTP_X_FORWARDED_FOR', 'HTTP_X_FORWARDED', 'HTTP_X_CLUSTER_CLIENT_IP', 'HTTP_FORWARDED_FOR', 'HTTP_FORWARDED', 'REMOTE_ADDR') as $key) {
		if (array_key_exists($key, $_SERVER) === true) {
			//foreach (explode(',', $_SERVER[$key]) as $ip) {
			$ips = array_reverse(explode(',', $_SERVER[$key])); // jwg - if multiple, last is for end user
			foreach ($ips as $ip) {
...
function get_commission($projectId,$amount,$projectType='0',$type='p')
{
	...
	else
	{
		...
		{	// 0,0 in commision table serves this case .. jwg
...
{
	$con=new DBconn();
	$sel_pro_basic=mysql_fetch_assoc($con->recordselect("SELECT * FROM projectbasics WHERE projectId='".$projectId."'"));
	...
	else
	{
		if($sel_pro_basic['rewardedAmount']>=$sel_pro_basic['fundingGoal']) {
			...
			cancel_preapprovals($projectId); // jwg
		}
	}
}

function cancel_preapprovals($projectId) // jwg
{	// called by daily cron job for projects that did not meet their funding in alotted time
	global $PayPalConfig;
	$con=new DBconn();
	$sel_backing_r = $con->recordselect("SELECT * FROM `projectbacking` WHERE `projectId` = '$projectId'");
	if (mysql_num_rows($sel_backing_r) == 0) {
		// nothing to cancel ...
	} else {
		$Pay=new Payment($PayPalConfig);
		// cancel preapproval for each backing case
		while ($sel_backing=mysql_fetch_assoc($sel_backing_r)) {
			// new sdk
			$Pay->doCancelPreapproval($sel_backing);
			
			// old sdk
			//$Pay->cancel_preapprovals($sel_backing);
		}	
		$Pay->cancel_preapprovals_creator_mail($projectId);
	}
}
...
function pay_to_backers_on_refund($projectId)
{
	...
	$creator_detail=mysql_fetch_assoc($con->recordselect("select pbs.projectId,pbs.projectTitle,u.paypalUserAccount,u.userId,u.emailAddress,u.name,pbs.preapproval_key,pbs.fundingGoalType from projectbasics as pbs,users as u,projects as p where pbs.projectId='".$projectId."' and u.userId=p.userId and pbs.projectId=p.projectId"));
	
	$final_array_creator=array(); // jwg .. this used to be $final_array -- changed to $final_array_creator
	//									    so it would not be replaced by backer stuff below...
	$final_array_creator['projectId'] =$creator_detail['projectId'];
	$final_array_creator['project_name'] = $creator_detail['projectTitle'];
	$final_array_creator['creator_id']=$creator_detail['userId'];
	$final_array_creator['creator_name']=$creator_detail['name'];
	$final_array_creator['creator_email']=base64_decode($creator_detail['emailAddress']);
	$final_array_creator['creator_paykey']=$creator_detail['preapproval_key'];
	$final_array_creator['creator_paypal']=base64_decode($creator_detail['paypalUserAccount']);
	$final_array_creator['fundingGoalType']=$creator_detail['fundingGoalType'];
	
	$userDetail_r=$con->recordselect("select pb.*,pbs.projectId,pbs.projectTitle,u.emailAddress,u.name from projectbasics as pbs,users as u, projectbacking as pb where pb.projectId='".$projectId."' and u.userId=pb.userId and pbs.projectId=pb.projectId and pb.payment_status='p' and pb.preapproval_key IS NOT NULL");
	...
	{				
		...
		$project_name,$creator_name,$creator_id,$amount,$commision,$rewardId,$backer_email,$creator_email*/
			...
			$final_array['creator_paypal']=base64_decode($creator_detail['paypalUserAccount']);
			$final_array['fundingGoalType']=$creator_detail['fundingGoalType'];
			
			$final_array['amount'] = $userDetails['pledgeAmount'];
			$final_array['commision'] = urlencode(get_commission($projectId,$userDetails['pledgeAmount'],$creator_detail["fundingGoalType"]));
			...
			// jwg 
			$paypalId = $userDetails['paypalId'];
			$transaction_r=$con->recordselect("SELECT * FROM `paypaltransaction` WHERE `paypalId` = $paypalId ");
			if(mysql_num_rows($transaction_r)>0)
			{				
				$transaction=mysql_fetch_assoc($transaction_r);
				$final_array['transactionId'] = $transaction['transactionId'];
				$final_array['correlationId'] = $transaction['correlationId'];
				$final_array['trackingId'] = $transaction['trackingId'];
			} else {
				$final_array['transactionId'] = '';
				$final_array['correlationId'] = '';
				$final_array['trackingId'] = '';
				wrtlog("WARNING - pay_to_backers_on_refund found no paypaltransaction record for {$final_array['backer_email']} for paypalId $paypalId ");
			}
			//print_r($final_array);
	...
	$Pay->payRefund_Unsuccessful_creator_mail($final_array_creator); // jwg (changed $final_array to $final_array_creator)
...
function pay_to_creator_on_success($projectId)
{
	...
	if(mysql_num_rows($userDetail_r)>0)
	{
		...
		$project_name,$creator_name,$creator_id,$amount,$commision,$rewardId,$backer_email,$creator_email*/
			...
			$final_array['commision'] = urlencode(get_commission($projectId,$userDetails['pledgeAmount'],'0','p')); // jwg .. forced '0','p' in call
			$final_array['paykey'] =$userDetails['preapproval_key'];
...
function remove_currency($string) { // jwg -- remove leading currency symbol from amount
									//     -- also remove internal commas
	$temp = preg_replace('#^\$#','',str_replace(',','',trim($string)));
	return $temp;
}

// jwg - turn object into associative array
