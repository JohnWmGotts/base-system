...
class PayPal_Adaptive extends PayPal
{
	...
	{	
		...
		if (!file_exists(DIR_FS.'/logs/paypal')) { // jwg
			mkdir(DIR_FS.'/logs/paypal',0775);
		}
		$file = DIR_FS."/logs/paypal/".$timestamp.$filename.".xml"; // jwg
		$fh = fopen($file, 'w'); 
	...
	{	
		//wrtlog("paypal.adaptive.class.php at ->Pay(..): DataArray=".print_r($DataArray,true));
		...
		//$Email = isset($Receivers['Email']) ? base64_decode($Receivers['Email']) : '';
		...
		//wrtlog("paypal.adaptive.class.php XMLResponse: ".print_r($XMLResponse,true));
		/*
		<?xml version='1.0' encoding='UTF-8'?>
		<ns2:PayResponse xmlns:ns2="http://svcs.paypal.com/types/ap">
			<responseEnvelope>
				<timestamp>2014-10-29T22:59:35.113-07:00</timestamp>
				<ack>Success</ack>
				<correlationId>40b0711d769b8</correlationId>
				<build>13414382</build>
			</responseEnvelope>
			<payKey>AP-3TM59912LV136723S</payKey>
		...
		*/
		...
		$ResponseDataArray = array(
								   ...
								   'TrackingID' => $TrackingID, // added by jwg to tie correlation id to notify later
								   'Timestamp' => $Timestamp, 
								   'PayKey' => $PayKey, 
	...
	{
		wrtlog("In ExecutePayment with: ".print_r($DataArray,true));
		...
		wrtlog("XMLResponse from ExecutePayment: ".print_r($XMLResponse,true));
	...
	{	
		...
		$XMLRequest .= $StartingDate != '' ? '<startingDate xmlns="">' . $StartingDate . '</startingDate>' : '';
			...
			$XMLRequest .= '</requestEnvelope>'; // jwg
	...
	function Refund($DataArray)
	{
		...
		$CorrelationID = isset($RefundFields['CorrelationID']) ? $RefundFields['CorrelationID'] : ''; // jwg
		...
		if(!empty($Receivers)) // jwg note - receivers is an empty array
		{
			$XMLRequest .= '<receiverList xmlns="">';
			foreach($Receivers as $Receiver)
			{
		...
		$XMLRequest .= $TransactionID != '' ? '<transactionId xmlns="">' . $TransactionID . '</transactionId>' : '';
		$XMLRequest .= $TrackingID != '' ? '<trackingId xmlns="">' . $TrackingID . '</trackingId>' : '';
		$XMLRequest .= $CorrelationID != '' ? '<correlationId xmlns="">' . $CorrelationID . '</correlationId>' : ''; // jwg
