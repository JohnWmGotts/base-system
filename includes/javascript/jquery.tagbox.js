...
(function(jQuery) {
  jQuery.fn.tagBox = function(options) {
    var defaults = {
      separator: ',',
      className: 'tagBox',
      tagInputClassName: 'input-text',
      tagButtonClassName: '',
      tagButtonTitle: 'Add',
      confirmRemoval: true,
      confirmRemovalText: 'Are you sure you want to delete?',
...
//  Hide Element
  ...
  //  Create DOM Elements
      if( (options.enableDropdown) && options.dropdownSource() != null ) {
        ...
        var $tag_input_elem = (options.readonly) ? '' : jQuery('<input name="web_site1" onfocus="entername();" type="text" class="'+options.tagInputClassName+' '+options.className+'-input" value="" id="'+options.className+'-input-'+uuid+'" />');
      }
      var $tag_add_elem = (options.readonly) ? '' : jQuery('<a href="javascript:void(0)" title="Add Website" class="'+options.tagButtonClassName+''+options.className+'-add-tag" id="'+options.className+'-add-tag-'+uuid+'">'+options.tagButtonTitle+'</a><div class="clear"></div> <label style="color:red; text-align:left;" id="web_arror"></label><br>');
     ...
     // var $tagBox = jQuery('<span class="'+options.className+'-container"></span>').append($tag_input_elem).append($tag_add_elem).append($tag_list_elem);
	  var $tagBox = jQuery('<span class="'+options.className+'-container"></span>').append($tag_input_elem).append($tag_add_elem);
     
      $(".input-tip").append($tag_list_elem);
  ...
  //  Methods
      ...
      function generate_tags_list(tags_list) {
        ...
        jQuery.each(tags_list, function(key, val) {
          if(val != "") {
            var remove_tag_link = (options.readonly) ? '' : '<a href="javascript:void(0)" class="'+options.className+'-remove '+options.className+'-remove-'+uuid+' remove" title="Remove Website" rel="'+val+'">'+options.removeTagText+'</a><input type="hidden" value="'+val+'" name="websiteadd[]">';
            ...
            $tag_list_elem.append('<table  class="tbl-website-width tbl-website" width="79%" align="right"><tr class="'+options.className+'-item-content"><td><font size="2">'+display_val+'</font></td><td><font size="2">'+remove_tag_link+'</font></td></tr></table>');
          }
        });
      ...
      function add_tag(new_tag_items) {
		...
		if(urlregex.test(new_tag_items)==false)
		{			
			document.getElementById('web_arror').innerHTML="Invalid format";
			return false;
		}
