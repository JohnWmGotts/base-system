...
$.widget( "ui.geo_autocomplete", {
	...
	options: {
		...
		mapheight: 80, // height of static map thumbnail
		maptype: 'hybrid', // see http://code.google.com/apis/maps/documentation/staticmaps/#MapTypes
		...
		delay: 30, // see http://jqueryui.com/demos/autocomplete/#option-delay
		...
		getItemHTML: function(_item) {		
			var _src = 'https://maps.google.com/maps/api/staticmap?visible=' + _item.viewport.getSouthWest().toUrlValue() + '|' + _item.viewport.getNorthEast().toUrlValue() + '&size=' + this.mapwidth + 'x' + this.mapheight + '&maptype=' + this.maptype + '&sensor=' + (this.mapsensor ? 'true' : 'false');
			return '<a><img style="float:left;margin-right:5px;" src="' + _src + '" width="' + this.mapwidth + '" height="' + this.mapheight + '" /> ' + _item.label.replace(/,/gi, ', ') + '<br clear="both" /></a>'
		}
	}
});